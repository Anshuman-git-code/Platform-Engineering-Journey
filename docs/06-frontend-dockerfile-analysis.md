...
| Stage 1 analysis — `FROM node:22-alpine AS builder` | Complete |
| Stage 1 analysis — `WORKDIR` | Complete |
| Stage 1 analysis — `COPY package.json package-lock.json` | Complete |
| Stage 1 analysis — `RUN npm ci` | Complete |
| `npm ci` vs `npm install` — engineering reasoning | Complete |
| `package.json` vs `package-lock.json` — lockfile contract | Complete |
| Stage 1 analysis — `COPY . .` | Complete |
| Stage 1 analysis — `RUN npm run build` | Complete |
| Node as compiler vs Node as runtime server | Complete |
| Stage 2 analysis — `FROM nginx:alpine` | Complete |
| Stage boundary — what Stage 1 discards | Complete |
| Stage 2 analysis — `COPY --from=builder` | Complete |
| Nginx document root — `/usr/share/nginx/html` | Complete |
| Stage 2 analysis — `EXPOSE 80` | Complete |
| Stage 2 analysis — `CMD ["nginx", "-g", "daemon off;"]` | Complete |
...
### Remaining — Phase 3 Practical

| Topic | Status |
|---|---|
| Frontend image build — execution and output analysis | Pending |
| Image size comparison — single-stage vs multi-stage | Pending |
| Container execution and browser verification | Pending |
| Internal container inspection | Pending |
| Debugging exercise | Pending |
